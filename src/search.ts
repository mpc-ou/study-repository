...
export function buildSearchIndex(
  ...
  index.export(function (key: string, data: string | null) {
    if (data !== null && data !== undefined) {
      exportData[key as string] = data;
    }
  });

  const manifest: { keys: string[]; totalEntries: number } = {
  ...
  for (const [key, data] of Object.entries(exportData)) {
    ...
    fs.writeFileSync(path.join(searchDir, `${safeKey}.json`), data, "utf-8");
  }
